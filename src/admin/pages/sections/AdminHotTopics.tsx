...
// const EMPTY = { title: '', subtitle: '', author: '', date: '', category: '', region: '', topic: 'Hot Topics', sortOrder: 0, isActive: true };
...
//       <h3 className="font-semibold text-gray-800">{init?._id ? 'Edit Article' : 'New Article'}</h3>
...
//         </div>
//         <div>
//           <label className="label">Date (e.g. March 27, 2026)</label>
...
//         </div>
//         <div>
//           <label className="label">Sort Order (1 = first shown)</label>
...
//         <button onClick={() => s('isActive', !f.isActive)}
...
//         <button onClick={() => onSave(f, file)} disabled={saving || !f.title}
...
// export function AdminHotTopics() {
...
//   const load = () => get('/home-articles/section/hot-topics').then(setArticles).finally(() => setLoading(false));
//   useEffect(() => { load(); }, []);

//   const toFD = (f: any, file: File | null) => {
//     const fd = new FormData();
//     fd.append('section', 'hot-topics');
...
//           <p className="text-sm text-gray-500">Topic cards shown in a 2×2 grid. Set Sort Order: 1 = top-left, 2 = top-right, etc.</p>
//         </div>
//         {!adding && (
//           <button onClick={() => setAdding(true)} className="rounded-xl bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700">+ Add Article</button>
//         )}
//       </div>
...
const EMPTY = {
  ...
  category: '', region: '', topic: 'Hot Topics', sortOrder: 0, isActive: true,
};
...
export function AdminHotTopics() {
  ...
  const load = () =>
    get('/home-articles/admin/all?section=hot-topics')
      .then(setArticles)
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Hot Topics</h2>
          <p className="text-sm text-gray-500">Cards shown in a 2×2 grid. Sort Order: 1 = top-left, 2 = top-right, etc.</p>
        </div>
        {!adding && (
